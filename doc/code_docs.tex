...
\begin{center}
  \begin{tabular}{|c|c|C{8cm}|}
    ...
    & \code{run} & \code{parent\_record} parameter to \code{execute\_step} \\
    \hline
    \multirow{4}{*}{implicit RunStep parameters} & \code{child\_run} & unset \\
    \cline{2-3}
    & \code{RSICs} & 1 in progress, 0 or more succesful/complete,
                     \newline \textit{or} \newline 
                     0 in progress, 1 or more succesful/complete \\
    \hline
    \multirow{4}{*}{inherited from RunAtomic} & \code{execrecord} & None \\
    & \code{reused} & None \\
    \cline{2-3}
...
\begin{center}
  \begin{tabular}{|c|c|C{8cm}|}
    ...
    & \code{RSICs} & all succesful and complete \\
    \hline
    \multirow{4}{*}{inherited from RunAtomic} & \code{execrecord} & unset \\
    \cline{2-3}
    & \code{reused} & None \\
    \cline{2-3}
    & \code{log} & None \\
...
\begin{center}
  \begin{tabular}{|c|c|C{8cm}|}
    ...
    & \code{run} & \code{parent\_record} parameter to \code{execute\_step} \\
    \hline
    \multirow{2}{*}{implicit RunStep parameters} & \code{child\_run} & Run created for sub-Pipeline \\
    \cline{2-3}
    & \code{RSICs} & all succesful and complete \\
    \hline
    \multirow{4}{*}{inherited from RunAtomic} & \code{execrecord} & unset \\
...
\begin{center}
  \begin{tabular}{|c|c|C{8cm}|}
    ...
    & \textbf{parameter} & \textbf{value} \\
    \hline
    \multirow{2}{*}{ExecLog parameters} & \code{record} & RunStep of executing step \\
    \cline{2-3}
    & \code{invoking\_record} & if recovering, \code{parent\_record} parameter
    to \code{execute\_step}, otherwise same as \code{record} \\
    \hline
    \multirow{2}{*}{implicit ExecLog parameters} & \code{integrity\_checks} & unset \\
    \cline{2-3}
    & \code{content\_checks} & unset \\
    \hline
    \multirow{2}{*}{inherited from Stopwatch} & \code{start\_time} & None \\
    \cline{2-3}
    & \code{end\_time} & None \\
    \hline
    \hline
    \multirow{4}{*}{MethodOutput parameters} & \code{execlog} & newly created ExecLog \\
    \cline{2-3}
