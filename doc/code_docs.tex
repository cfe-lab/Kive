...
Some time later, Jim decides that the first output of his Pipeline wasn't quite
what he wanted. He creates a new Pipeline with an additional step, $D$, to do
some more processing to the first output (Figure~\ref{fig:newpipeline}).
...
%To illustrate this, consider step $E$ of Jim's Pipeline above. Suppose the
%input to this step has a CompoundDatatype called ``site\_probability'', which
%he has defined as (integer: position, float: probability). In other words, the
%site\_probability CompoundDatatype has two columns: an integer column called
%``position'', and a float column called ``probability''. Because the cable
%linking steps $A$ and $E$ is trivial, we may infer that the SymbolicDataset
%$A_1$ also has the same CompoundDatatype.
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture} [node distance=\nodedist]
    ...
    \node [below right=of step1out, text width=3cm] (outlabel) {TransformationOutput \\ output from $A$ \\ input to $c_2$};
