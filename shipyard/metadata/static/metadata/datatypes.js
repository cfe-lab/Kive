...
$(document).ready(function(){ // wait for page to finish loading before executing jQuery code
    ...
    for (var i = 0; i < 1 + numberOfExtraForms; i++) {
    ...
    $("#addForm").click(  // query button by id selector
        function () {   // anonymous function
            numberOfExtraForms += 1;
            renderForms(numberOfExtraForms);
        }
    )
    $("#removeForm").click(
        function() {
            if (numberOfExtraForms > 0) {
                numberOfExtraForms -= 1;
                renderForms(numberOfExtraForms);
            }
        }
    ...
    var renderForms = function($nForms) {
        var htmlStr = "";
        for (var i = 1; i < $nForms + 1; i++) {
            // generate drop-down menus
            htmlStr += "<tr>";
            htmlStr += "<td>"+(i+1)+"</td>"; // column index
