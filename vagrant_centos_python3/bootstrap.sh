...
${PYTHON} -m venv kive_venv
source kive_venv/bin/activate

cd /usr/local/share/Kive/api
python setup.py install  # we are inside the virtualenv now so we just call the virtualenv Python
cd ..
mkdir --parents \
...
pip install -r requirements-dev.py34.txt  # pip is installed inside the virtualenv
