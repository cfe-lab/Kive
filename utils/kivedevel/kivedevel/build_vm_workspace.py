...
def workspace_rsync_args(root: Path, workdir: Path, destination: Path) -> list[str]:
...
def _attach_container_workspace(cmds: Cmds, instance: str, root: Path, workdir: Path) -> None:
    ...
    cmds.rsync.run(workspace_rsync_args(root, workdir, host_mount_dir))
...
def _build_and_attach_vm_workspace(cmds: Cmds, instance: str, image_path: Path, root: Path, workdir: Path) -> None:
    ...
    if not image_path.exists():
        ...
        sudo("mount", "--", "/dev/nbd0", str(mount_dir))

        cmds.rsync.run(workspace_rsync_args(root, workdir, mount_dir), sudo=True)
