...
def main():
    ...
    for run in runs:
        while True:
            ...
            unfinished_run_count = sum(1
                                       for run in active_runs
                                       if not run['end_time'])
            if unfinished_run_count < args.active_count:
                break
            sleep(10)

        run_outputs = kive.get(run['run_outputs']).json()
        logger.debug('Rerunning %s', run['display_name'])
        for run_output in run_outputs['output_summary']:
            ...
            try:
                ...
                ex.args = (patch_result.json()['detail'], )
                raise
        ...
        try:
            kive.post('@api_runs', json=params, is_json=True)
        except Exception:
